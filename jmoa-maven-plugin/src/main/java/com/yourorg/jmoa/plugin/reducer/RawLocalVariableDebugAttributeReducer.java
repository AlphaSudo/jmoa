...
/**
 * Removes LocalVariableTable and LocalVariableTypeTable by rewriting only Code
 * attribute headers. All constant-pool entries and unrelated attributes are
 * copied from the original byte stream.
 */
public final class RawLocalVariableDebugAttributeReducer {

    public byte[] reduce(byte[] originalBytes) {
        return new Reducer(originalBytes).reduce();
    }
    ...
    private static final class Reducer {
        ...
        private void writeReducedMethod(ByteArrayOutputStream out) {
            ...
            for (int i = 0; i < attributes; i++) {
                ...
                if ("Code".equals(name)) {
                    attributeBytes.writeBytes(rewriteCodeAttribute(nameIndex, infoOffset, checkedLength(length)));
                } else {
            ...
            writeU2(out, newAttributeCount);
            out.writeBytes(attributeBytes.toByteArray());
        }

        private byte[] rewriteCodeAttribute(int nameIndex, int infoOffset, int length) {
